...
// ---------- Admin ----------
// One shared password grants "admin" to whichever browser enters it —
// there's no real user-identity system underneath this, same tradeoff
// as the account cookies. Set your own via the ADMIN_PASSWORD
// environment variable before deploying; the fallback below is only
// for local testing.
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'dev-admin-change-me';

// adminAccounts: Set<accountId> — accounts that have entered the password
const adminAccounts = new Set();

// Site-wide settings an admin can change without touching code.
// NOTE: this lives only in memory — it resets if the server restarts
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && req.url === '/site-settings') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  if (req.method === 'GET' && req.url === '/admin/status') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ isAdmin: adminAccounts.has(accountId) }));
    return;
  }
  ...
  if (req.method === 'POST' && req.url === '/admin/settings') {
    if (!adminAccounts.has(accountId)) {
    ...
    req.on('end', () => {
      ...
      siteSettings = {
        title: String(parsed.title || siteSettings.title).slice(0, 100),
        bgImage: String(parsed.bgImage || '').slice(0, 500)
      };
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify(siteSettings));
    });
